...
Framebuffer::Framebuffer(int rows, int columns, int parallel)
  ...
  Clear();
  assert(rows_ <= 32);
  assert(parallel >= 1 && parallel <= 5); // change
...
/* static */ void Framebuffer::InitGPIO(GPIO *io, int parallel) {
  ...
  if (parallel >= 4) {								// change
    b.bits.p3_r1 = b.bits.p3_g1 = b.bits.p3_b1 = 1;
    b.bits.p3_r2 = b.bits.p3_g2 = b.bits.p3_b2 = 1;
  }

  if (parallel >= 5) {
    b.bits.p4_r1 = b.bits.p4_g1 = b.bits.p4_b1 = 1;
    b.bits.p4_r2 = b.bits.p4_g2 = b.bits.p4_b2 = 1;
  }
...
#ifndef ONLY_SINGLE_CHAIN // change
    plane_bits.bits.p1_r1 = plane_bits.bits.p1_r2 =
      plane_bits.bits.p2_r1 = plane_bits.bits.p2_r2 =
    	plane_bits.bits.p3_r1 = plane_bits.bits.p3_r2 =
    	  plane_bits.bits.p4_r1 = plane_bits.bits.p4_r2 = (red & mask) == mask;
    plane_bits.bits.p1_g1 = plane_bits.bits.p1_g2 =
      plane_bits.bits.p2_g1 = plane_bits.bits.p2_g2 =
	    plane_bits.bits.p3_g1 = plane_bits.bits.p3_g2 =
		  plane_bits.bits.p4_g1 = plane_bits.bits.p4_g2 = (green & mask) == mask;
    plane_bits.bits.p1_b1 = plane_bits.bits.p1_b2 =
      plane_bits.bits.p2_b1 = plane_bits.bits.p2_b2 =
	    plane_bits.bits.p3_b1 = plane_bits.bits.p3_b2 =
	      plane_bits.bits.p4_b1 = plane_bits.bits.p4_b2 = (blue & mask) == mask;
#endif
...
void Framebuffer::SetPixel(int x, int y, uint8_t r, uint8_t g, uint8_t b) {
  ...
  // ---------------------------------------------------------------------------------------------------------
  else if (y >= 3*rows_ && y < 4 * rows_) { // change - 4th row
     // Parallel chain #4
     if (y - 3*rows_ < double_rows_) {   // Upper sub-panel.
       for (int b = min_bit_plane; b < kBitPlanes; ++b) {
         const uint16_t mask = 1 << b;
         bits->bits.p3_r1 = (red & mask) == mask;
         bits->bits.p3_g1 = (green & mask) == mask;
         bits->bits.p3_b1 = (blue & mask) == mask;
         bits += columns_;
       }
     } else {
       for (int b = min_bit_plane; b < kBitPlanes; ++b) {
         const uint16_t mask = 1 << b;
         bits->bits.p3_r2 = (red & mask) == mask;
         bits->bits.p3_g2 = (green & mask) == mask;
         bits->bits.p3_b2 = (blue & mask) == mask;
         bits += columns_;
       }
     }
   }

  else if (y >= 4*rows_ && y < 5 * rows_) { // change - 5th row
      // Parallel chain #5
      if (y - 4*rows_ < double_rows_) {   // Upper sub-panel.
        for (int b = min_bit_plane; b < kBitPlanes; ++b) {
          const uint16_t mask = 1 << b;
          bits->bits.p4_r1 = (red & mask) == mask;
          bits->bits.p4_g1 = (green & mask) == mask;
          bits->bits.p4_b1 = (blue & mask) == mask;
          bits += columns_;
        }
      } else {
        for (int b = min_bit_plane; b < kBitPlanes; ++b) {
          const uint16_t mask = 1 << b;
          bits->bits.p4_r2 = (red & mask) == mask;
          bits->bits.p4_g2 = (green & mask) == mask;
          bits->bits.p4_b2 = (blue & mask) == mask;
          bits += columns_;
        }
      }
    }
...
void Framebuffer::DumpToMatrix(GPIO *io) {
  ...
  if (parallel_ >= 4) {			// change
    color_clk_mask.bits.p3_r1
      = color_clk_mask.bits.p3_g1
      = color_clk_mask.bits.p3_b1
      = color_clk_mask.bits.p3_r2
      = color_clk_mask.bits.p3_g2
      = color_clk_mask.bits.p3_b2 = 1;
  }

  if (parallel_ >= 5) {			// change
    color_clk_mask.bits.p4_r1
      = color_clk_mask.bits.p4_g1
      = color_clk_mask.bits.p4_b1
      = color_clk_mask.bits.p4_r2
      = color_clk_mask.bits.p4_g2
      = color_clk_mask.bits.p4_b2 = 1;
  }
#endif
